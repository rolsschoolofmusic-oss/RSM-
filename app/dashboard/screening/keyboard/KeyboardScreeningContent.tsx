...
const KB_NOTATION_OPTIONS = ["Staff Notation", "Chord Charts / Tabs", "Numbers / Solfa", "None"] as const;
...
const LM_QUESTIONS: TrackTestQuestion[] = [
  {
    id: "lm-01", code: "LM-01", title: "Key Press Sensitivity",
    sub: "Invite the child to press a single key on the keyboard and observe their reaction. Do not prompt a second attempt.",
    options: SENSORY_OPTIONS,
    optionDescs: {
      Positive: "Reaches out readily, curious or pleased.",
      Neutral: "Presses without a strong reaction either way.",
  ...
  },
  {
    ...
    sub: "Teacher taps a pulse on the keyboard body — observe if the child joins in.",
  ...
  {
    id: "lm-03", code: "LM-03", title: "Key Reach & Exploration Curiosity",
    sub: "Let the child freely explore the keys under supervision.",
    options: GRADE_OPTIONS,
    optionDescs: {
...
const JT_QUESTIONS: TrackTestQuestion[] = [
  {
    id: "jt-01", code: "JT-01", title: "Seated Posture at Keyboard",
    sub: "Observe posture and comfort while seated at the keyboard.",
    options: SENSORY_OPTIONS,
    optionDescs: {
  ...
  },
  {
    ...
    sub: "Ask the student to spread all fingers wide over the keys and hold for 5 seconds. Observe comfort, range, and any tension.",
    options: SENSORY_OPTIONS,
    optionDescs: {
...
const CT_QUESTIONS: TrackTestQuestion[] = [
  {
    id: "ct-01", code: "CT-01", title: "Key Press Sensitivity",
    sub: "Play a single soft note (pp) then a forte note (ff) — observe the student's response. Present each dynamic level without naming it; watch for flinching, covering ears, or leaning in.",
    options: SENSORY_OPTIONS,
    optionDescs: {
      Positive: "Engaged, curious, or visibly enjoys the contrast.",
  ...
  {
    id: "ct-02", code: "CT-02", title: "Timbre & Sound Texture",
    sub: "Switch between Piano, Strings, and Vibraphone patches, playing the same 3-note phrase in each. Observe facial expression and any verbal or physical reaction to the change in texture.",
    options: SENSORY_OPTIONS,
    optionDescs: {
      Positive: "Engaged, curious, or visibly enjoys the variety.",
      Neutral: "Attentive without a strong reaction.",
      Withdrawal: "Mild discomfort or disinterest with certain timbres.",
      Distress: "Strong aversive reaction to a timbre change.",
    },
  },
  {
    id: "ct-03", code: "CT-03", title: "Rhythmic Pulse Sync",
    sub: "Tap a steady 60 BPM pulse on the keyboard body and invite the student to join. Use palm taps on the casing — observe whether the student mirrors, delays, avoids, or ignores the pattern.",
    options: SENSORY_OPTIONS,
    optionDescs: {
...
function computeJtConfig(answers: string[]): KeyboardConfig {
...
export function KeyboardScreeningContent({ onBack }: { onBack?: () => void }) {
  ...
  useEffect(() => {
    ...
    getTrackQuestionBank("keyboard", "lmQuestions").then(qs => {
      if (qs && qs.length > 0) { setLmQuestions(qs); setLmDraftQuestions(qs); }
    }).catch(() => {});
    getTrackQuestionBank("keyboard", "jtQuestions").then(qs => {
      if (qs && qs.length > 0) { setJtQuestions(qs); setJtDraftQuestions(qs); }
    }).catch(() => {});
    getTrackQuestionBank("keyboard", "ctQuestions").then(qs => {
                      ...
                      <>
                        ...
                        <button type="button" disabled={lmBankSaving}
                          onClick={async () => {
                            ...
                            try {
                              await saveTrackQuestionBank("keyboard", "lmQuestions", lmDraftQuestions, user?.uid ?? "unknown");
                      ...
                      <>
                        ...
                        <button type="button" disabled={jtBankSaving}
                          onClick={async () => {
                            ...
                            try {
                              await saveTrackQuestionBank("keyboard", "jtQuestions", jtDraftQuestions, user?.uid ?? "unknown");
                      ...
                      <>
                        ...
                        <button type="button" disabled={ctBankSaving}
                          onClick={async () => {
                            ...
                            try {
                              await saveTrackQuestionBank("keyboard", "ctQuestions", ctDraftQuestions, user?.uid ?? "unknown");
